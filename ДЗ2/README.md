...
# Core Domain Chart
## Переход "Менеджмент" в Generic в будущем
**Reasoning:**
Основная функция менеджмента на первых порах - собрать фидбек по заказам. 
Пока такое работает из-за того, что все клиенты - это коты-тестировщики внутри системы. 
Когда пользователей будет много, лучше будет автоматизировать сбор отзывов о работе.
# Сравнение
...
**Делегирование:** scalability, availability, agility, testability, deployability, modifiability, evolvability
...
# Спорные моменты
- Возможно, подсчет стоимости тоже будет другим поддоменом. 
  Это все же про деньги :)
- Проверка качества по идее может влиять на матчинг, но при этом матчингом занимается команда исследователей, а проверкой качества - менеджеры.
  Хотелось бы объединить
- "Подсчет стоимости" может быть отдельным поддоменом, частью матчинга или частью работы с заказом. Но пока стоимость константа, поэтому непонятно.
